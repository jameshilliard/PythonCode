...
Usage :
    python SetDSLAM_Calix.py [-l <VDSL>] [-t tag] [-b] [-m <8A|8B|8C|12A>] [-d]
"""
__author__ = "pwang"
__version__ = "1.0"
__history__ = """
Rev 1.0 : 2013/06/27
    Initial version for Calix DSLAM
"""
import os, sys, re
import time
...
class SetDSLAM_Calix():
    ...
    def __init__(self, linemode='', tag='', bonding=False, vdslmode='', testbed='', adslmode='', pvc='', connectport='',
        ...
        #            print 'AT_ERROR : Calix DSLAM only support ADSL mode!'
        #            exit(1)
        if profile_name:
    ...
    def set(self):
        """
        """
        print '\n' + '#' * 50 + 'Begin to Set Calix DSLAM configuration!'
        if self.login():
            pass
        else:
            return False
        ...
        if self.delete_vlan_transfer(self._dslam_primary_port):
            pass
        else:
            self.logout(False)
            return False
        ...
        if self._dslam_second_port:
            if self.delete_vlan_transfer(self._dslam_second_port):
                pass
            else:
                self.logout(False)
                return False
        ...
        if self.bond_vlan_dsl(self._vlan, self._using_port, self._tag):
            pass
        else:
            self.logout(False)
            return False
        ...
        if self._linemode == 'ADSL':
            ...
            if self._tag:
                if self.set_vlan_transfer(self._using_port, self._vlan, self._tag):
                    pass
                else:
                    self.logout(False)
                    return False
            else:
                if self.bond_vlan_pvid(self._using_port, self._vlan):
                    pass
                else:
                    self.logout(False)
                    return False
            if self._bonding:
                if self.set_work_mode(self._dslam_primary_port, self._profile_name, self._vdslmode):
                    pass
                else:
                    self.logout(False)
                    return False
                if self.set_work_mode(self._dslam_second_port, self._profile_name, self._vdslmode):
                    pass
                else:
                    self.logout(False)
                    return False
                ...
                if self.set_work_mode(self._using_port, self._profile_name, self._vdslmode):
                    pass
                else:
                    self.logout(False)
                    return False
        else:
            print 'AT_ERROR : linemode must be VDSL or ADSL!'
            self.logout(False)
            return False
        self.save()
        self.logout(True)
    ...
    def remove(self):
        """
        """
        print '\n' + '#' * 50 + 'Begin to Remove Calix DSLAM configuration!'
        if self.login():
            pass
        else:
            return False
    ...
    def check_profile(self, name):
        ...
        cmd = 'vdsl profile show ' + profile_name
        self._child.sendline(cmd)
        try:
            index = self._child.expect([self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
            res = self._child.before
            ...
            #                rc = os.system('echo "' + res + '"|grep ' + '"not exist"')
            #                if rc == 0:
            #                    cmd = 'vdsl profile set ' + profile_name + ' interleave 512 2048 6 0 31 32 6 0 31 64 3 9 3 9 '
            #                    self._child.sendline(cmd)
            #                    index = self._child.expect([':', self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
            #                    if index == 1:
            #                        pass
        ...
        while True:
            ...
            try:
                ...
                if rc == 0:
                    ...
                    rc = os.system('echo "' + res + '"|grep "' + 'referred by member"')
                    if rc == 0:
                        cmd = 'vdsl pvc show'
                        self._child.sendline(cmd)
                        index = self._child.expect([self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
                        ...
                        (rc, result) = self.subproc('echo "' + res + '"|grep "' + ':"')
                        if rc == 0:
                            self.error(cmd)
                            return False
                        else:
    ...
    def bond_vlan_pvid(self, port, vid):
        """
        switch vlan pvid 19 505
        """
        print '\n' + '#' * 50 + 'Enter function bond_vlan_pvid'
        vlan = str(vid)
        port = str(port)
        cmd = 'switch vlan pvid ' + port + ' ' + vlan
        self._child.sendline(cmd)
        try:
            index = self._child.expect([': ', self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
            if index == 1:
            ...
            return False
        self.info(cmd)
        print '\nAT_INFO : bond_vlan_pvid Successfully!'
        return True
    ...
    def bond_vlan_dsl(self, vid, port, tag):
        """
        ...
        tag = str(tag)
        if tag:
            cmd = 'switch vlan set ' + vlan + ' ' + port + ':FT'
        else:
        ...
        if tag:
            cmd = 'switch vlan set ' + tag + ' ' + port + ':FT'
            self._child.sendline(cmd)
            try:
                index = self._child.expect([': ', self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
                if index == 1:
    ...
    def delete_vlan_transfer(self, port):
        """
        vdsl vtt show 20
        vdsl vtt del 20 999 0 201
        port   vpi  vci  cxvid  <->  svid   cvid 
        ------ ---- ---- -------     ------ ------
            14    *    *     998          0    203
            14    *    *     999          0    201

        """
        print '\n' + '#' * 50 + 'Enter function delete_vlan_transfer'
        port = str(port)
        while True:
            cmd = ' vdsl vtt show ' + port
            self._child.sendline(cmd)
            try:
                index = self._child.expect([self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
                res = self._child.before
                ...
                self.error(cmd)
                return False
            rc = os.system('echo "' + res + '"|grep ' + '"^ *' + port + '  *"')
            if rc == 0:
                (rc, result) = self.subproc(
                    'echo "' + res + '"|grep ' + '"^ *' + port + '  *"' + "|head -n1|awk '{print $4 " + '" ' + '" $5 "' + ' " $6}' + "'")
                result = result.strip()
                print 'result', result
                reresult = '[0-9]+  *0  *[0-9]+'
                rc = re.findall(reresult, result)
                if rc:
                    cmd = 'vdsl vtt del ' + port + ' ' + result
                    self._child.sendline(cmd)
                    try:
                        index = self._child.expect([self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
                        res = self._child.before
                        ...
                        self.error(cmd)
                        return False
                    rc = os.system('echo "' + res + '"|grep -E "' + 'Error|invalid|usage:"')
                    if rc == 0:
                        print 'AT_ERROR : ' + cmd + ' ERROR!'
                        return False
                    else:
                        pass
                else:
                    print 'AT_ERROR : vdsl vtt show format error'
                    self.error(cmd)
                    return False

            else:
                print 'AT_INFO : Port ' + port + ' was not set vlan transfer!'
                break
        print '\nAT_INFO : delete_vlan_transfer Successfully!'
        self.info(cmd)
        return True

    def set_vlan_transfer(self, port, vlan, tagID):
        """
        vdsl vtt show 20
        vdsl vtt set 20 999 0 201
        port   vpi  vci  cxvid  <->  svid   cvid 
        ------ ---- ---- -------     ------ ------
            14    *    *     998          0    203
            14    *    *     999          0    201
        ...
        """
        print '\n' + '#' * 50 + 'Enter function set_vlan_transfer'
        port = str(port)
        vlan = str(vlan)
        tagID = str(tagID)
        cmd = 'vdsl vtt set ' + port + ' ' + vlan + ' 0 ' + tagID
        self._child.sendline(cmd)
        try:
            index = self._child.expect([self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
            res = self._child.before
        ...
        if rc == 0:
            ...
            return False
        print '\nAT_INFO : set_vlan_transfer Successfully!'
        self.info(cmd)
        return True

    def delete_pvc_port(self, port):
        """
        vdsl pvc delete 11 0 35
        """
        ...
        for p in port_list:
            if p:
                rc = os.system('echo "' + res + '"|grep ' + '"^ *' + str(p) + '  *"')
                if rc == 0:
    ...
    #        self._child.sendline(cmd)
    #        try:
    #            index = self._child.expect(['no such channel.*\r\n(config changed)ras> *$', 'invalid command', 'usage:', self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
    #            print self._child
    ...
    def bond_pvc_port(self, port, vpi, vci, vid):
        """
        vdsl pvc set 11 0 35 vlan 504 0 DEFVAL llc
        """
        ...
        vpi = str(vpi)
        vci = str(vci)
        cmd = 'vdsl pvc set ' + port + ' ' + vpi + ' ' + vci + ' ' + vlan + ' 0 DEFVAL llc'
        self._child.sendline(cmd)
        try:
            index = self._child.expect([':', self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
            if index == 1:
    ...
    def delete_bond_group(self, name):
        """
        vdsl gbond delete bg7
        """
        print '\n' + '#' * 50 + 'Enter function delete_bond_group'
        bond_group_name = str(name)
        cmd = 'vdsl gbond delete ' + bond_group_name
        self._child.sendline(cmd)
        try:
            index = self._child.expect([self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
            print self._child
            res = self._child.before
            rc = os.system('echo "' + res + '"|grep "' + 'no such bonding group"')
            if rc == 0:
                pass
            else:
                rc = os.system('echo "' + res + '"|grep "' + ':"')
                if rc == 0:
                    self.error(cmd)
                    return False
                else:
    ...
    def add_bond_group(self, name, mport, sport):
        ...
        sport = str(sport)
        while True:
            cmd = 'vdsl gbond set ' + bond_group_name + ' ' + mport + ',' + sport
            self._child.sendline(cmd)
            try:
                index = self._child.expect([self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
                print self._child
                res = self._child.before
                command1 = 'echo "' + res + '"|grep "' + "group member is referenced" + '"'
    ...
    def backup_optionmask(self, port):
        """
        vdsl optionmask set 2 0x0000
        """
        print '\n' + '#' * 50 + 'Enter function backup_optionmask'
        port_list = port
        for p in port_list:
            if p:
                cmd = 'vdsl optionmask set ' + str(p) + ' 0x0000'
                self._child.sendline(cmd)
                try:
                    index = self._child.expect([':', self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
                    if index == 1:
                    ...
                    return False
        self.info(cmd)
        print '\nAT_INFO : backup_optionmask successfully!'
        return True

    def enable_annexm(self, port):
        """
        vdsl optionmask set 2 0x0400
        """
        print '\n' + '#' * 50 + 'Enter function enable_annexm'
        port = str(port)
        cmd = 'vdsl optionmask set ' + port + ' 0x0400'
        self._child.sendline(cmd)
        try:
            index = self._child.expect([':', self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
            if index == 1:
    ...
    def set_work_mode(self, port, profile, module):
        """
        vdsl profile map 19 ben111 adsl2+ 
        """
        ...
        if str(module) == 'annexm':
            amoduler = 'adsl2+'
        cmd = 'vdsl profile map ' + port + ' ' + dslprofile + ' ' + amoduler
        self._child.sendline(cmd)
        try:
            index = self._child.expect([':', self._base_prompt, pexpect.EOF, pexpect.TIMEOUT])
            if index == 1:
...
def main():
    ...
    main
    """
    usage = "python SetDSLAM_Calix.py [-l <ADSL|VDSL>] [-t tag] [-b] [-a <adsl2+|adsl2|gdmt|glite|t1413|annexm>] [-v <17a|12a|12b|8a|8b|8c|8d>] [-x <pvc 0/32>] [-P <1|2>] [-d]"
    ...
    rc = False
    if remove:
        setdslam = SetDSLAM_Calix(linemode, tag, bonding, vdslmode, testbed, adslmode, pvc, connectport)
        rc = setdslam.remove()
    else:
        setdslam = SetDSLAM_Calix(linemode, tag, bonding, vdslmode, testbed, adslmode, pvc, connectport)
        rc = setdslam.set()
