...
class TestLink(TestlinkAPIClient):
    ...
    def getTestCaseIDByName(self, testCaseName, testSuiteName, testProjectName):
        ...
        if results[0].has_key("message"):
            ...
            raise TestLinkError(
                "(getTestCaseIDByName) - Several case test found. Suite name must not be duplicate for the same project")
        else:
    ...
    def getBuildByName(self, testProjectName, testPlanName, buildName):
        ...
        if builds == '':
            raise TestLinkError(
                "(getBuildByName) - Builds %s does not exists for test plan %s" % (buildName, testPlanName))
