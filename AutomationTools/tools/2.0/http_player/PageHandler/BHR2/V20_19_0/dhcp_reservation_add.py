...
class Page(PageBase):
    ...
    def replBody(self, body):
        ...
        print '====>', reservation_ip
        for k in range(0, 4):
            body.updateValue('ip' + str(int(k)), reservation_ip.split('.')[k])
        lan_eth1_mac = os.getenv('G_HOST_MAC0_1_0')
        lan_eth2_mac = os.getenv('G_HOST_MAC0_2_0')
        print '====>', lan_eth1_mac, lan_eth2_mac
        for index in range(0, 6):
            print '---->', index
            body.updateValue('mac' + str(int(index)), lan_eth1_mac.split(':')[index])
        print '========>2', body
        return body
