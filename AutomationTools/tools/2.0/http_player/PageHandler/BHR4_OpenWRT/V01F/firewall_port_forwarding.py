            ...
            if str(is_apf) == '':
                ...
                for index, k in enumerate(fmt['keys']):
                    ...
                    if k == 'firewall_forward_localhost_list':
                        ...
                        continue
                    elif k == 'firewall_forward_protocol_ports_custom':
                        ev = new_eStart_add + '-' + new_eEnd_add
                        if new_eStart_add != new_eEnd_add:
                            if ev:
                                if v != '':
                                    v = ev
                                    print '== firewall_forward_protocol_ports_custom : ', v
                            body.updateValueByIndex(index, v)
                        elif new_eStart_add == new_eEnd_add:
                            ev = new_eStart_add
                            v = ev
                            print '== firewall_forward_protocol_ports_custom : ', v
                            body.updateValueByIndex(index, v)
                        ...
                        if ev:
                            if v != '':
                                v = ev
                                print '== firewall_forward_destports_advanced : ', v
                        body.updateValueByIndex(index, v)
                        continue
                    #	elif k == 'firewall_forward_sourceports_advanced' :
                    #		ev = new_eStart_add + '-' + new_eEnd_add
                    #		if ev :
                    #			if v != '':
                    #				#v = ev
                    #				v = ''
                    #				print '== firewall_forward_sourceports_advanced : ', v
                    #		body.updateValueByIndex(index, v)
                    #		continue
