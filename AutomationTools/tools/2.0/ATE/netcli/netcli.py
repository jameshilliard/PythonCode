...
class TelnetHandler(SocketServer.BaseRequestHandler):
    ...
    def setup(self):
        ...
        time.sleep(0.5)
        #
        self.writeline('Welcome my Telnet Server!')
    ...
    def cmdCLI(self, params):
        """
        Run cli command
        """
        resp = os.popen(' '.join(params) + ' 2>&1').read()
        self.sendline(resp)
        #cmd = ' '.join(params)
        #status,output=commands.getstatusoutput(cmd) 

    def cmdSH(self, params):
        """
        Run shell command
        """
        resp = os.popen(' '.join(params) + ' 2>&1').read()
...
def callback(socket, cmd, opt):
    ...
    if cmd in [WILL, WONT, DO, DONT]:
        ...
        opttxt = ""
        #print("==>OPTION: %s %s" % (cmdtxt, opttxt, ))
...
class NetCLI():
    ...
    def Run(self, HOST, PORT, handler=TelnetHandler, start_client=True):
        """
        """
...
if __name__ == '__main__':
    ...
    #print dir(curses)
    # Multi
    server = ThreadedTCPServer((HOST, PORT), TNH)
    server_thread = threading.Thread(target=server.serve_forever)
