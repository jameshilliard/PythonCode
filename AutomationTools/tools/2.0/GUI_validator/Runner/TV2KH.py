...
class Runner_31_122L_11d(BaseRunner):
    """
    """
    driver = None
    base_url = None
    ...
    def restore_default(self):
        ...
        if self.goto(page_uri):
            ...
            elem_reset = self.wait_elem_enabled(how='xpath',
                                                what="//div[@id='content_right_contentarea']/table/tbody/tr[5]/td[2]/a/span")
            if elem_reset:
                elem_reset.click()
            else:
                return False
            ...
            if not self.waiting_page('Reboot Info', 'Actiontec'):
                if self.driver.title == 'Problem loading page':
                    print 'Warning : page crashed , try to refresh it'
                    self.driver.refresh()
                    return self.check_title('Actiontec', 60)
                else:
                    return False
            else:
                return True
    ...
    def tr_setting(self):
        ...
        page_uri = 'advancedutilities_tr069.html'
        ...
        if self.goto(page_uri):
            ...
            elem_enable = self.wait_elem_enabled(how='name', what="ACS_Debug")
            if elem_enable:
                elem_enable.click()
            else:
                return False
    ...
    def wan_layer2_setting(self, params={}):
        ...
        if self.goto(page_uri):
            ...
            #############################################################################
            elem_applybtn = self.wait_elem_enabled(how='id', what='applyBtn')
            if elem_applybtn:
                elem_applybtn.click()
            else:
                return False
...
hash_runners = {
    '31.122L.11d': Runner_31_122L_11d,
}

def_runner = '31.122L.11d'
