...
{
    echo "CLI setting : set ACS_URL to http://xatechdmw.xdev.motive.com/cwmpWeb/CPEMgt and change ACS_PeriodicInformInterval to 60"
    perl $U_PATH_TBIN/DUTCmd.pl -o cli_1st_setting.log -l $G_CURRENTLOG -d $G_PROD_IP_BR0_0_0 -v "cli -s InternetGatewayDevice.ManagementServer.URL string http://xatechdmw.xdev.motive.com/cwmpWeb/CPEMgt" -v "cli -s InternetGatewayDevice.ManagementServer.PeriodicInformInterval int 60" -u $U_DUT_TELNET_USER -p $U_DUT_TELNET_PWD
    if [ $? -ne 0 ] ;then
