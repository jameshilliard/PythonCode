...
#22 May 2013    |   2.0.0   | Andy      | Support new Motive Server (HDM4.0)
############################################################################################
...
do
    ...
    -test)
        ...
        G_PFVERSION=2.0
        U_DUT_TYPE=TV2KH
        U_DUT_FW_VERSION=31.122L.01
        U_PATH_TBIN=./
        G_CURRENTLOG=/dev/shm
        G_HOST_USR1=root
        G_HOST_PWD1=actiontec
        G_PROD_IP_BR0_0_0=192.168.1.254
        G_HOST_TIP1_0_0=192.168.172.62
        G_HOST_IF1_1_0=eth1
        U_DUT_TELNET_USER=admin
        U_DUT_TELNET_PWD=paswword
        U_AUTO_CONF_BIN=playback_http
        U_AUTO_CONF_PARAM="-d 0"
        U_DUT_HTTP_USER=root
        U_DUT_HTTP_PWD="=kjW:k"
        U_CUSTOM_MOTIVE_CLIENT_VER=4.0
...
function check_authorization
{
    ...
    do
        ...
        head=`echo $ACS_URL | grep -o "xatechdm\."`
        if [ "$head" = "xatechdm." ]; then
...
# Support new Motive Server (HDM4.0)
function post_1st_file_4
{
    get_cwmp
    echo "GUI setting : set ACS_URL to http://iiothdmw13.iot.motive.com/cwmpWeb/CPEMgt and change ACS_PeriodicInformInterval to 60 by post file B-GEN-ENV.PRE-DUT.TR069CONF-002-C001"
    $U_AUTO_CONF_BIN $U_DUT_TYPE $post_file_loc/B-GEN-ENV.PRE-DUT.TR069CONF-002-C001
    if [ $? -ne 0 ] ;then
        echo "AT_ERROR : FAILED to $U_AUTO_CONF_BIN $U_DUT_TYPE $post_file_loc/B-GEN-ENV.PRE-DUT.TR069CONF-002-C001"
        stop_capture_and_exit 1
    fi
}
...
function check_authorization_4
{
    ...
    do
        ...
        ACS_URL=`grep "TMP_DUT_CWMP_ACS_URL" $G_CURRENTLOG/cwmpInfo.log | awk -F= '{print $2}'`
        echo "ACS_URL=$ACS_URL"
        head=`echo $ACS_URL | grep -o "iiothdm13"`
        if [ "$head" = "iiothdm13" ]; then
            echo "ACS_URL has been changed by Motice"
            get_cwmp_enable=0
        else
            if [ $try -ge $trytimes ]; then
...
function post_2nd_file_4
{
    get_cwmp
    if [ "$U_CUSTOM_CWMP_FORCE_PERIODIC_INFORM" == "0" -a "$U_CUSTOM_MOTIVE_SSH_SERVER_IPADDRESS" == "$G_HOST_IP1" ] ;then
        echo "GUI setting : change ACS_PeriodicInformInterval to 86400 by post file B-GEN-ENV.PRE-DUT.TR069CONF-002-C003"
        $U_AUTO_CONF_BIN $U_DUT_TYPE $post_file_loc/B-GEN-ENV.PRE-DUT.TR069CONF-002-C003
    else
        echo "GUI setting : change ACS_PeriodicInformInterval to 60 by post file B-GEN-ENV.PRE-DUT.TR069CONF-002-C002"
        $U_AUTO_CONF_BIN $U_DUT_TYPE $post_file_loc/B-GEN-ENV.PRE-DUT.TR069CONF-002-C002
    fi
    if [ $? -ne 0 ] ;then
        echo "AT_ERROR : FAILED to $U_AUTO_CONF_BIN $U_DUT_TYPE $post_file_loc/B-GEN-ENV.PRE-DUT.TR069CONF-002-C00X"
        stop_capture_and_exit 1
    fi
}
...
if [ "$U_CUSTOM_MOTIVE_CLIENT_VER" == "3.0" ] ;then
    ...
    python $U_PATH_TBIN/TR069_config.py
