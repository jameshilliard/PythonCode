...
rc=$?
if [ $rc -eq 0 ] ;then
    #bash $U_PATH_TBIN/acquireDefaultConnectionService_jacs.sh $*
    #exit $?
    exit 0
fi
####date for test#############
...
#InternetGatewayDevice.Layer3Forwarding.DefaultConnectionService = InternetGatewayDevice.WANDevice.1.WANConnectionDevice.10.WANPPPConnection.1
    ...
    if [ $rc -eq 1 ] ;then
        ...
        echo $res | awk '{print $3}'| awk '-F.' '{print "U_TR069_DEFAULT_CONNECTION_SERVICE=" $0" ""U_TR069_WANDEVICE_INDEX=" $1"."$2"."$3 " ""U_TR069_WANCONNECTIONDEVICE_INDEX=" $4"."$5 " U_TMP_WAN_CONNECTION_TYPE=Coax"}' | tee $G_CURRENTLOG/$output
    else
        echo "AT_WARNING : acquir current active connection type failed, WAN device index is $rc"
    fi
    exit 0
