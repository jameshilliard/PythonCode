...
#	lan_table.each {|x| print x}
	landuplexmode=@ff.element_by_xpath('/html/body/form/table/tbody/tr[3]/td/table/tbody/tr/td[2]/table/tbody/tr[2]/td[2]/table/tbody/tr/td/table/tbody/tr[2]/td[2]/table[2]/tbody/tr/td/table/tbody/tr[7]/td[2]')
	puts "Mac address in GUI is #{landuplexmode}"
	puts "Expect MAC address is #{$expect}"
        if "#{landuplexmode}".strip == "#{$expect}".strip then
                puts "------------------------------------------------------\n"
                puts "PASS: The duplex mode of LAN interface is '#{$expect}'\n"
                puts "------------------------------------------------------\n"
		return true
        else
                puts "------------------------------------------------------\n"
                puts "FAIL: The duplex mode of LAN interface is #{landuplexmode}"
                puts "------------------------------------------------------\n"
		return false
        end
