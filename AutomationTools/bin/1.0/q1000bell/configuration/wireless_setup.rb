...
module WirelessSetup
    def basic_settings
        ...
        if @user_choices.member?(:wireless)
            @ff.radio(:id, "id_wl_on").set if @user_choices[:wireless]
            @ff.radio(:id, "id_wl_off").set unless @user_choices[:wireless]
        ...
        if @ff.radio(:id, "id_wl_on").checked?
            ...
            @log.info("Basic Wireless::Changed SSID to #{@user_choices[:wireless_ssid]}")
