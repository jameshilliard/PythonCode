...
class Page(PageBase):
    ...
    def replBody(self, body):
        ...
        for index, k in enumerate(fmt['keys']):
            v = fmt['vals'][index]
            if k == 'wlFltMacAddr_wl0v0' or k == 'wlFltMacAddr_wl0v1' or k == 'wlFltMacAddr_wl0v2' or k == 'wlFltMacAddr_wl0v3':
                print '== original val :', str(v)
                if str(v) != '':
                    ev = os.getenv('U_WIRELESSCARD_MAC')
                    if ev:
                        v = ev
