...
body_fmts["action="] = 'action=remove_url&rmLst=www.vosky.com&needthankyou=advancedsetup_websiteblocking.html'
#body_fmts[""] = ''
#-----------------------------------------------------------------------
query_fmts = {}
...
class Page(PageBase):
    ...
    def replBody(self, body):
        ...
        #'rmLstIp':'G_HOST_TIP0_1_0',
        #'Lan_IP':'G_HOST_TIP0_1_0',
        fmt = body.fmt()

        for index, k in enumerate(fmt['keys']):
            v = fmt['vals'][index]
            if k == 'Lan_IP':
                ev_ori = body.value('Lan_IP')

                if ev_ori != 'all':
                    ev = os.getenv('G_HOST_TIP0_1_0')
                    if ev:
                        v = ev
                        print '== change LAN IP to : ', v
                    body.updateValueByIndex(index, v)
                continue
            elif k == 'rmLstIp':
                ev_ori = body.value('rmLstIp')

                if ev_ori != 'all':
                    ev = os.getenv('G_HOST_TIP0_1_0')
                    if ev:
                        v = ev
                        print '== change rmLstIp to : ', v
                    body.updateValueByIndex(index, v)
                continue
