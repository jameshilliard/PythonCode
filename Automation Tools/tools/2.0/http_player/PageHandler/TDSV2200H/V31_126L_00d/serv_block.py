...
class Page(PageBase):
    ...
    def replBody(self, body):
        ...
        for index, k in enumerate(fmt['keys']):
            v = fmt['vals'][index]
            if k == 'rules':
                sbl_device = os.getenv('G_HOST_TIP0_1_0')
                sbl_type = os.getenv('TMP_CURRENT_SBL_TYPE').replace('+', ' ')
                if sbl_type == 'HTTP':
                    ev = sbl_device + '/16|'
                elif sbl_type == 'FTP':
                    ev = sbl_device + '/8|'
                elif sbl_type == 'Newsgroups':
                    ev = sbl_device + '/4|'
                elif sbl_type == 'E-Mail':
                    ev = sbl_device + '/2|'
                elif sbl_type == 'Instant Messaging':
                    ev = sbl_device + '/1|'
                if ev:
                    v = ev
