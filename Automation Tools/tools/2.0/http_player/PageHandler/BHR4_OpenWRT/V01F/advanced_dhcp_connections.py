...
class Page(PageBase):
    ...
    def replBody(self, body):
        ...
        for index, k in enumerate(fmt['keys']):
            v = fmt['vals'][index]
            if k == 'advanced_dhcpconnection_index':
                opt_type = body.value('sumbit_button_name')
                if opt_type == 'delete':
                    delindex = os.getenv('LAN_DHCP_RULE_INDEX', 'NULL_NULL')
                    if delindex != 'NULL_NULL':
                        body.updateValueByIndex(index, delindex)
                        continue
            elif k == 'advanced_dhcpconnection_ipaddress':
                ev = os.getenv('U_DUT_CUSTOM_LAN_RESERVATION_IP')
                if ev:
                    v = ev
                body.updateValueByIndex(index, v)
                continue
            elif k == 'advanced_dhcpconnection_macaddress':
