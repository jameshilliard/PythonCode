...
class BaseTest(object):
    ...
    def getClassFromTest(self):
        ...
        classes = {'benchmark': ['unicast_latency',
                   ...
                   'roaming':   ['roaming_delay', 'roaming_benchmark',],
                   'qos':       ['qos_capacity',
                                 'qos_service',
                                 'voip_roam_quality',
                                 ],
    ...
    def loadData(self, 
        ...
        attribute_dict_db_chassis={}
        attribute_dict_db_testspec ={}
        attribute_dict_db_client_table_eth={}
        attribute_dict_db_client_table_wifi={}
        atribute_dict_db_sec={}
        atribute_dict_db_traffic={}
        atribute_dict_db_logsandresults={}
        atribute_dict_db_testparams ={}
        attribute_dict_db_mapping={}
        global dict_testcase
        dict_testcase={}
        dict_testcase=waveTestStore
        ## Loading  the Dictionaries   
        attribute_dict_db_chassis.update(waveChassisStore.values()[0].values()[1].values()[0].items())
        attribute_dict_db_testspec.update(waveTestSpecificStore.values()[0].items()) 
        #attribute_dict_db_client_table_eth.update(waveClientTableStore.values()[0].items())
        attribute_dict_db_client_table_wifi.update(waveClientTableStore.values()[0].items())
        ## a small hack to get the security info for wireless group
        tmp=[]
        tmp = waveSecurityStore.values()
        for each in tmp:
            if len (each.values()) > 3: 
                atribute_dict_db_sec.update(each.items())  
            else:
                pass
        atribute_dict_db_traffic.update(waveTestStore.values()[0].items() )
        atribute_dict_db_logsandresults.update(waveTestStore.values()[3].items())
        atribute_dict_db_testparams.update(waveTestStore.values()[4].items())
        attribute_dict_db_map={} 
        for each_pa in waveMappingStore[1:5]:
                index=waveMappingStore.index(each_pa)
                attribute_dict_db_map[index]=each_pa
        attribute_dict_db_mapping.update(attribute_dict_db_map.items())
        each_list_db=[] 
        ## Converting Dict-->list of Tuples for Ordering in DataBase Tables
        iteration_tuple=(attribute_dict_db_chassis, attribute_dict_db_testspec,attribute_dict_db_client_table_eth,attribute_dict_db_client_table_wifi,atribute_dict_db_sec,atribute_dict_db_traffic,atribute_dict_db_logsandresults,atribute_dict_db_testparams,attribute_dict_db_mapping)
        for each_dict in iteration_tuple :
            for colname,colvalue in each_dict.items():
                each_list_db=each_list_db+[(colname,colvalue)]
            self.attribute_final_list_db=self.attribute_final_list_db+each_list_db   
        try:
           if  waveTestStore['LogsAndResultsInfo']['db']:
               ...
               if  waveTestStore['LogsAndResultsInfo']['dbusername']:
               	   self.DbUser = waveTestStore['LogsAndResultsInfo']['dbusername']
               else:
                   self.DbUser = "veriwave"
    ...
    def CloseShop(self): 
        ...
        #___________________________________DataExport___________________________________ 
            ...
            if  dict_testcase['LogsAndResultsInfo']['TestCaseName']:
                  TC_dict['TestCaseName']= waveTestStore['LogsAndResultsInfo']['TestCaseName'] 
            else:
                  try:  
                  ...
                  #import subprocess
                  #subprocess.call(["rm","-f","temp_tc.txt"]) 
            if dict_testcase['LogsAndResultsInfo']['TestCaseDescription']:
                  TC_dict['Description']=waveTestStore['LogsAndResultsInfo']['TestCaseDescription']
            else:
...
#                try:
                   ...
                   db_instance =vw_dataExport.ExportData(self.DbServerIP,self.DbType,self.DbName,self.DbUser,self.DbPassword)
...
class WaveReport:
    ...
    def InsertUserspecifiedTable(self, data1, columns=[]):
       ...
       """
       #print "DATATATATAToteh datat coming is %s" %data1  
       if self.Story == None:
            return
       from reportlab.lib import colors
       styles = getSampleStyleSheet()
