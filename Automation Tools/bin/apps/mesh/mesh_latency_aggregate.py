...
#
# Objective: In a mesh network, measure the latency of all hops at the
...
class Test(MeshCommon, BaseTest):
    def __init__(self):
        ...
        """
        self.CSVfilename      = 'Results_mesh_latency_aggregate.csv'
        self.ReportFilename   = 'Report_mesh_latency_aggregate.pdf'
        self.LoggingDirectory = "logs"
        self.SavePCAPfile     = False
        self.DetailedFilename = 'Detailed_mesh_latency_aggregate.csv'
...
########################## DO NOT MODIFY BELOW HERE ##########################
        # Include the version 
        self.version = '$Revision: 1.17 $' 
        self.date    = '$Date: 2007/07/13 18:32:09 $' 
    ...
    def loadData( self,
        ...
        if  waveTestStore['LogsAndResultsInfo'].has_key('pf'):
            if  waveTestStore['LogsAndResultsInfo']['pf'] == "True":
                ...
                if waveTestSpecificStore['mesh_latency_aggregate']['AcceptableMaxLatency']:
                    if float (waveTestSpecificStore['mesh_latency_aggregate']['AcceptableMaxLatency']) >= 0:
                        self.UserPassFailCriteria['ref_max_latency']=float(waveTestSpecificStore['mesh_latency_aggregate']['AcceptableMaxLatency'])/1000
                    else:
                ...
                if waveTestSpecificStore['mesh_latency_aggregate']['AcceptableAvgLatency']:
                    if float (waveTestSpecificStore['mesh_latency_aggregate']['AcceptableAvgLatency']) >= 0:
                        self.UserPassFailCriteria['ref_avg_latency']=float(waveTestSpecificStore['mesh_latency_aggregate']['AcceptableAvgLatency'])/1000
                    else:
        ...
        iLoadlist = []
        frameSizeList = []
        iLoadList = waveTestSpecificStore['mesh_latency_aggregate']['ILoadList']
        frameSizeList = waveTestSpecificStore['mesh_latency_aggregate']['FrameSizeList']
        if (len(iLoadList) != 0):
            self.ILOADlist = []
    ...
    def startTest(self, FuncRealTime):
        ...
        for InterationIndex in range(len(self.FrameSizeList)):
            ...
            for TrialNumber in range(1, 1 + self.Trials):                                 
                WaveEngine.ModifyFlows(self.FlowList, {'FrameSize': FrameSize, 
                ...
                if self.UserPassFailCriteria['user']=="True": 
                        CSVline = ('Frame Size', 'ILOAD pkt/sec', 'Trial', 'Minimum Latency',
                           'Maximum Latency', 'Average Latency', 'Average Jitter(us)','USC:AVGLAT','USC:MAXLAT')
                else:
                        CSVline = ('Frame Size', 'ILOAD pkt/sec', 'Trial', 'Minimum Latency', 
                           'Maximum Latency', 'Average Latency', 'Average Jitter(us)')
                ...
                if self.UserPassFailCriteria['user']=="True":
                  ...
                  if latencyMin == 0.0 or latencyMax == 0.0 or latencyAvg == 0.0:
                            ...
                            WaveEngine.OutputstreamHDL("\nThe test failed , so applying P/F criteria is invalid", WaveEngine.MSG_WARNING)
                  else:
        ...
        self.RealtimeData['Frame Size']        = 0
        self.RealtimeData['Intended Load FPS'] = 0
    ...
    def PrintReport(self):
        ...
        Overview1 = "The Aggregate Latency test determines the overall latency of "\
                    "the mesh network for the system under test (SUT). The test "\
                    "presents the SUT with an intended load (ILOAD) and measures the "\
                    "time that it takes for frames to be forwarded through the SUT "\
                    "(latency) across all the nodes in the mesh network. To understand "\
                    "how latency varies with different types of traffic, you can set "\
                    "up different ILOADS and frame sizes. Each combination of ILOAD "\
                    "and frame size is tested separately."
        Overview2 = "If multiple nodes are involved in the test, the ILOAD is divided "\
                    "evenly across the nodes; if multiple clients are associated with "\
                    "a node, the ILOAD for that node is divided evenly between the "\
                    "clients."
        Overview3 = "Latency is measured by taking the difference between the arrival "\
                    "time of a packet at the destination port and the time the packet "\
                    "was sent at the source port. Frames are transmitted for a fixed "\
                    "period of time. The difference between the transmit time and the "\
                    "receive time is the latency."
        Overview4 = "The results show the latencies for each frame size as distributed "\
                    "into 16 latency buckets. Each latency bucket has a maximum "\
                    "latency time boundary, which are set up in ascending order. "\
                    "The arriving packets are placed in the last latency bucket "\
                    "for which their latency value does not exceed the latency bucket "\
                    "boundary." 
        Overview5 = "For accurate latency measurement, the ILOAD must be at a level "\
                    "that produces no frame loss. Use the Aggregate Throughput test "\
                    "to determine the maximum ILOAD that can be achieved without "\
                    "frame loss."      
        ...
        JitterText= "The average jitter graph is a statistical manipulation of each of the frame's latency "\
                    ...
                    "averaged together."
        HistogramText = "The latency distribution graphs shows the amount of variation and consistency in "\
                        "packet latency. There are 16 different latency time segments or buckets. Each "\
                        "bucket has an upper time boundary representing the maximum latency value for "\
                        "that bucket. The test measure the number of packets whose latency time falls "\
                        "within each bucket."
        ...
        Method1 = "The test associates test clients with the SUT ports, including "\
                  ...
                  "The test traffic is applied across all nodes in the mesh network. "\
                  "Proprietary timestamps inserted in each test traffic frame are "\
                  "then used to calculate the minimum, maximum and average latency "\
                  "as per RFC 2544, as well as the smoothed inter-arrival jitter "\
                  "according to RFC 3550." 
        Method2 = "Each combination of ILOAD and frame size is tested separately. "\
                  "If multiple nodes are involved in the test, the ILOAD is divided "\
                  "evenly across the nodes; if multiple clients are associated with "\
                  "a node, the ILOAD for that node is divided evenly between the "\
                  "clients."
        Method3 = "Latency measurements are made accurately even in the presence "\
                  "of frame loss. However, the ILOAD should be set such that no "\
                  "frame loss occurs; otherwise, buffer occupancy delays can obscure "\
                  "actual SUT datapath delays. The throughput test may be used to "\
                  "determine this traffic level."
        Method4 = "The results are recorded separately for each combination of test "\
                  "conditions, as well as for each trial if multiple trials are run. "\
                  "Results from multiple trials are averaged into the graphs in previous "\
                  "sections."                     
        ...
        if MyReport.Story == None:
            ...
            return
        MyReport.Title("Mesh Aggregate Latency Report", self.DUTinfo, self.TestID)
        ...
        MyReport.InsertParagraph(Overview5)        
        self.ParseCVSdata(self.ResultsForCSVfile)
        GraphWidth  = 6.0 * inch
        GraphHeight = 3.5 * inch
        MyReport.InsertObject(self.CreateLatencySummaryGraph(GraphWidth, GraphHeight, self._LatencyList, self._LatencyData))
        MyReport.InsertParagraph(LatencyText)
        #MyReport.InsertPageBreak()

        MyReport.InsertHeader("Jitter Graph")
        MyReport.InsertParagraph(JitterText)
        MyReport.InsertObject(self.CreateLatencySingleGraph(GraphWidth, GraphHeight, GraphTypes.Jitter, self._LatencyList, self._LatencyData))

        #MyReport.InsertPageBreak()
        MyReport.InsertHeader("Latency Distribution")
        ...
        MyReport.InsertPageBreak(2.0*inch) #Break if there is less than an inch left.
        ...
        if self.UserPassFailCriteria['user']=="True":
             MyReport.InsertDetailedTable(self._LatencyResults, columns=[0.5*inch, 0.8*inch, 0.5*inch, 0.8*inch, 0.8*inch, 0.8*inch, 0.8*inch,1*inch,1*inch])
             ...
             for each_tuple in self._LatencyResults:
                    ...
                    if each_tuple[-1] == 'FAIL' or  each_tuple[-2] == 'FAIL':
 ...
 the input from the user for maximum and average allowable latency in (ms), based on that we will evaluate the test result as PASS/FAIL. """
             ...
             MyReport.InsertDetailedTable(self._LatencyResults, columns=[0.5*inch, 0.8*inch, 0.5*inch, 0.8*inch, 0.8*inch, 0.8*inch, 0.8*inch])
...
#            self._clientLearning(self.ListOfClients, self.ClientLearningTime, self.ClientLearningRate)
    ...
    def getInfo(self):
        msg = """The Aggregate Latency test determines the overall latency of the mesh network for the system under test (SUT). The test presents the SUT with an intended load (ILOAD) and measures the time that it takes for frames to be forwarded through the SUT (latency) across all the nodes in the mesh network."""         
    ...
    def getCharts(self):
        ...
        #Create the latency summary graph
        c = self.CreateLatencySummaryGraph(640, 480,self._LatencyList, self._LatencyData)
        t= c.title
        charts[ t ] = c

        #Create the Average Jitter Graph
        c = self.CreateLatencySingleGraph(640, 480,GraphTypes.Jitter,self._LatencyList, self._LatencyData)
        t = c.title
        charts[ t ] = c
        
        #Create the latency histogram graphs
        for (Names, Data, Title, LatencyMin, LatencyMax, LatencyAvg) in self._HistogramData:
...
# These are unique objects that a placed in the Report.PDF file.
    def CreateLatencySummaryGraph(self, GraphWidth, GraphHeight, IndexList, Resultdata):
        ...
        for eachIndex in IndexList:
            (FrameSize, FrameRate) = eachIndex
            (TotalMin, TotalMax, TotalAvg, Jitter, Count) = Resultdata[eachIndex]
    ...
    def CreateLatencySingleGraph(self, GraphWidth, GraphHeight, DisplayValue, IndexList, Resultdata):
        ...
        for eachIndex in IndexList:
            ...
            (TotalMin, TotalMax, TotalAvg, Jitter, Count) = Resultdata[eachIndex]
            if DisplayValue == GraphTypes.Maximum:
                FrameSizeData += (float(TotalMax) / float(Count), )
            elif DisplayValue == GraphTypes.Minimum:
                FrameSizeData += (float(TotalMin) / float(Count), )
            elif DisplayValue == GraphTypes.Average:
                FrameSizeData += (float(TotalAvg) / float(Count), )
            elif DisplayValue == GraphTypes.Jitter:
                FrameSizeData += (float(Jitter)/ 1000000.0 / float(Count), )
            else:
                FrameSizeData += (int(Count), )
            FrameSizeName.append("%d bytes @\n%spkt/sec" % (FrameSize, Float2EngNotation(FrameRate, self.DisplayPrecision)) )

        return self.LatencyGraph2(GraphWidth, GraphHeight, FrameSizeName, [ FrameSizeData, ], graphTitle)
    ...
    class LatencyGraph2(FlowableGraph):
        def __init__(self, width, height, names, bar, title):
            FlowableGraph.__init__(self, width, height)
            self.dataNames = names
            self.dataBar   = bar
