...
#        driver.find_element_by_id("apply_btn").click()
        if self.login() :
            ...
            driver.find_element_by_id("disabled").click()
            config_file_path = self.get_config_file_path()
            #"/tmp/config/PK5001AConfig-2013-02-20"
            driver.find_element_by_id("SysconfigUpload").send_keys(config_file_path)
            driver.find_element_by_link_text("Apply").click()
            driver.find_element_by_link_text("OK").click()

        self.check_url("http://192.168.0.1/login.html", 240)
