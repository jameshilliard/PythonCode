#######################################################
#
#   checkDHCPServerConfigure.rb
#   Author: Aleon
#   Date:   2010.7.22
#   Discription:    Display the status of DHCP server configuration
#   Input:  it depends
...
class Initialize_BHR2
	...
	if !@ff.text_field(:xpath,'//*[@id="dhcp_mode"]').disabled()
	    ...
	    opt = @ff.select_list(:name,'dhcp_mode').selected_options()
	    puts "----------------\n"
	    puts opt[0]
	    puts "----------------\n"

	    if opt[0] == $expect then
		puts "------------------------------------------------------\n"
		puts "PASS: The configuration of DHCP Server is '#{$expect}'\n"
		puts "------------------------------------------------------\n"
	    else
		puts "------------------------------------------------------\n"
		puts "FAIL: The configuration of DHCP Server is " + " " + opt[0] + "\n"
		puts "------------------------------------------------------\n"
	    end
