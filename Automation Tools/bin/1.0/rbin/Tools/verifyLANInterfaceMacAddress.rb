#######################################################
#
#   verifyLANInterfaceMACAddress.rb
#   Author: Aleon
#   Date:   2010.8.8
#   Discription:    verify whether the MAC address of lan interface is get correctly by motive
#   		   	 with GUI.
#   Input:  it depends
...
#	lan_table=@ff.bodies()
#	lan_table.each {|x| print x}
	lanMacAddress=@ff.element_by_xpath('/html/body/form/table/tbody/tr[3]/td/table/tbody/tr/td[2]/table/tbody/tr[2]/td[2]/table/tbody/tr/td/table/tbody/tr[2]/td[2]/table[2]/tbody/tr/td/table/tbody/tr[7]/td[2]')
	puts "Mac address in GUI is #{lanMacAddress}"
	puts "Expect MAC address is #{$expect}"
        if "#{lanMacAddress}".strip == "#{$expect}".strip then
                puts "------------------------------------------------------\n"
                puts "PASS: The MAC address of LAN interface is '#{$expect}'\n"
                puts "------------------------------------------------------\n"
		return true
        else
                puts "------------------------------------------------------\n"
                puts "FAIL: The MAC address of LAN interface is #{lanMacAddress}"
                puts "------------------------------------------------------\n"
		return false
        end
...
begin
	...
	case opt
	    ...
	    when '-h'
		puts "Usage: verifyLANInterfaceMacAddress.rb -d <=ipAddress> -u <=username> -p <=password> -e <=expectValue}> -h "
		exit 1
	    end
	end
end
