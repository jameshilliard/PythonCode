...
#     
#     Input:          checkDynamicDNSStatus.rb -f <$jsonfile=#{$jsonfile}> -h <help=#{$help}> 
#     Output:         Return the result of operation
#     
################################################################
...
class CheckHostnameNumb
	...
	File.open("#{$jsonfile}",'r') do |f1|
            ...
            while line = f1.gets
                ...
                if (line =~ /"Security Security Log Buffer Size":"*"/) != nil then

                    value = line.split"\""
                    get_expect = value[3].to_s
                    puts "Get the setting of buf is : #{get_expect}"
                end
            end # end of loop;
		...
		# Check the real value and compare with get_expect;
      		value = @ff.text_field(:name,'security_buf_size').value().to_s
