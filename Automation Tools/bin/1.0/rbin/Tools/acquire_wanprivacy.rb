...
class AcquireText < Essentialdut
  ...
  def privacy_status
    ...
    ret_value = @ff.checkbox(:name, 'clink_privacy').checked
    
    if ret_value.to_s == @compare_str
	puts "PASS Privacy value is #{ret_value} in GUI"
    else 
	puts "FAIL Privacy value is #{ret_value} in GUI"
    end
