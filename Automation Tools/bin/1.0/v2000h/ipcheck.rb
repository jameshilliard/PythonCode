...
module IPCommon
    ...
    def ip_by_interface(interface)
        return `ifconfig #{interface} | awk '/inet addr/ {split ($2,A,":"); print A[2]}'`.chomp
    end
...
class IP
	...
	def initialize(ip=nil)
        ...
        @url = ""
		holder = ""
        ip.match(/eth/i) ? new_ip = IPCommon::ip_by_interface(ip) : new_ip = ip
		unless new_ip == nil
	...
	def is_private?
		if (IPCommon::ip_int("10.0.0.0")..IPCommon::ip_int("10.255.255.255")) === IPCommon::ip_int(@ip)
			return TRUE
		elsif (IPCommon::ip_int("172.16.0.0")..IPCommon::ip_int("172.31.255.255")) === IPCommon::ip_int(@ip)
			return TRUE
		elsif (IPCommon::ip_int("192.168.0.0")..IPCommon::ip_int("192.168.255.255")) === IPCommon::ip_int(@ip)
			return TRUE
		end
		return FALSE
	end
	...
	def ip_class
		ipclass = ''
		ipclass = 'A' if (IPCommon::ip_int("10.0.0.0")..IPCommon::ip_int("127.255.255.255")) === IPCommon::ip_int(@ip)
		ipclass = 'B' if (IPCommon::ip_int("128.0.0.0")..IPCommon::ip_int("191.255.255.255")) === IPCommon::ip_int(@ip)
		ipclass = 'C' if (IPCommon::ip_int("192.0.0.0")..IPCommon::ip_int("223.255.255.255")) === IPCommon::ip_int(@ip)
		ipclass = 'D' if (IPCommon::ip_int("224.0.0.0")..IPCommon::ip_int("239.255.255.255")) === IPCommon::ip_int(@ip)
		ipclass = 'E' if (IPCommon::ip_int("240.0.0.0")..IPCommon::ip_int("255.255.255.255")) === IPCommon::ip_int(@ip)
		ipclass
	end
...
class String
    def valid_ip?
		...
		if IPCommon::ip_int(self) > 2**32-1 || IPCommon::ip_int(self) <= 0
			return FALSE
		else
			return TRUE
		end
