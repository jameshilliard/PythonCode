...
# record their output to a log directory $XXXX.log ( e.g $G_SQAROOT/logs ) 
# 
#CVS Tag:
#$Date:$
#$Revision:$
...
#		    pipe (PARENT_RDR,CHILD_WTR);
	...
	if ($noerrorcheck == $OFF) {
		...
		$rc = `grep -i -e "failed[^a-z]" -e "error[^a-z]"  $testlog_tcase | wc`;
		@resultF = split(" ",$rc);
...
#    $log->info($temp2);
	...
	$log->info("Start to execute step\[$index\]");
	if ( $profFile->{debug} > 4 ) {
	...
	foreach $kk ( keys %{$stagePtr}) {
	    $tc = $stagePtr->{$kk};
	    if ( $stagePtr->{$kk} =~ /HASH/) {
		$temp2 = " Step\[$index\] --- Entry  is not DEFINED ---";
		$log->warn($temp2);
...
[B<-l> I<logdir (defaults:$G_SQAROOT/logs)>]
...
1. The following command is used to run a list of testcases from command line
         perl gflaunch.pl  -v BUILD=/main/projects/builder/autobuild/official/trunk-ver_4r2b4/vf2113/vf2113_mos_v4r2b4.img -v G_USER=jnguyen -tc $G_SQAROOT/platform/$G_PFVERSION/testcases/sample/tc1.xml -tc $G_SQAROOT/platform/$G_PFVERSION/testcases/sample/tc2.xml -v G_USER=int-sqa
