...
class Configure < UserChoices::Command
    ...
    def add_choices(builder)
        ...
        # Wireless settings
        builder.add_choice(:wireless, :type=>:string) { |command_line| command_line.uses_option("--wireless STATE", "Turns wireless on or off") }
        ...
        builder.add_choice(:broadband_settings, :type=>[:string]) { |command_line| command_line.uses_option("--broadband_settings DEVICE,OPTIONS", "Configures broadband settings for DEVICE with OPTIONS. DEVICE can be [ptm], or [atm]. Options vary per device. For PTM: [vlan:[id_number]], [priority:priority_number], [mode:mode_type]. For ATM: [mode:mode_type], [vpi:value], [vci:value], [qos:qos_type], [pcr:value], [scr:value], [mbs:value], [cdvt:value], [encaps:[llc_bridged|llc_routed|llc/snap|vcmux]]") }
        builder.add_choice(:broadband_settings_mtu) { |command_line| command_line.uses_option("--broadband_settings_mtu MTU", "Sets the MTU value") }
        builder.add_choice(:broadband_settings_encapsulation) { |command_line| command_line.uses_option("--broadband_settings_encapsulation ENCAPSULATION", "Sets the encapsulation to 'bridged', 'routed', or 'vc-mu'x") }
        ...
        builder.add_choice(:dynamic_dns, :type=>[:string]) { |command_line| command_line.uses_option("--dynamic_dns USER,PASS,HOSTNAME", "Enable Dynamic DNS with the specified username, password, and hostname. Use 'disable' to disable dynamic DNS") }
        ...
        builder.add_choice(:igmp_proxy, :type=>:boolean) { |command_line| command_line.uses_switch("--igmp_proxy", "Enable or disable IGMP Proxy") }
        builder.add_choice(:qos_upstream, :type=>[:string]) { |command_line| command_line.uses_option("--qos_upstream OPTIONS", "List of comma separated options: [name:string], [priority:high|med|low], [reserve:bandwidth], [protocol:protocol], [tos:bit], [source:ip/netmask:port-range], [destination:ip/netmask:port-range]. Use 'disable' to disable upstream, or 'default' to enable and set to Qwest Default QoS") }
        builder.add_choice(:qos_downstream, :type=>[:string]) { |command_line| command_line.uses_option("--qos_downstream OPTIONS", "List of comma separated options: [name:string], [priority:high|med|low], [reserve:bandwidth], [protocol:protocol], [tos:bit], [source:ip/netmask:port-range], [destination:ip/netmask:port-range]. Use 'disable' to disable downstream, or 'default' to enable and set to Qwest Default QoS") }
    ...
    def config
        ...
        begin
            ...
            # wireless
            self.basic_settings if @user_choices[:wireless]
            ...
            self.website_blocking if @user_choices[:scheduling_access_add] || @user_choices[:scheduling_access_remove]
            self.broadband_settings if @user_choices[:broadband_settings] || @user_choices[:broadband_settings_mtu] || @user_choices[:broadband_settings_encapsulation]
            self.dhcp_settings if @user_choices[:dhcp_settings] || @user_choices[:dhcp_lease_time] || @user_choices[:dhcp_dns] || @user_choices[:lan_ip_address]
            ...
            self.igmp_proxy if @user_choices.member?(:igmp_proxy)
            self.qos_upstream if @user_choices.member?(:qos_upstream)
            self.qos_downstream if @user_choices.member?(:qos_downstream)
            self.remote_gui if @user_choices[:remote_gui] || @user_choices[:gui_info] || @user_choices[:remote_gui_timeout]
            self.remote_telnet if !@user_choices[:remote_telnet].nil? || @user_choices[:telnet_info] || @user_choices[:remote_telnet_timeout]
