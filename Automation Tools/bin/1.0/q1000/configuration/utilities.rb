...
module Utilities
    def reboot
        # @ff.js_eval("var win_all = getWindows(); var target_win = win_all[0]; target_win.content.doReboot(0)")
    ...
    def ping_test
        ...
        @log.debug("Ping Test::Running test. Pinging #{@ff.text_field(:id, "url").value} with packet size #{@ff.text_field(:id, "packet_size").value}")
        @ff.link(:id, "test_btn").click
        self.please_wait
        table_count = 2
        results = []
        results << @ff.elements_by_xpath("/html/body/div/div[3]/div/form/div[10]/div[5]/table[1]/tbody/tr/th").join("\t")
        results.last.insert(results.last.index("\t"), "\t")
        while table_count < 6
            results << @ff.elements_by_xpath("/html/body/div/div[3]/div/form/div[10]/div[5]/table[1]/tbody/tr[#{table_count}]/td").join("\t").delete(":")
            table_count += 1
        end
        results << @ff.elements_by_xpath("/html/body/div/div[3]/div/form/div[10]/div[5]/table[2]/tbody/tr/th").join("\t")
        results << @ff.elements_by_xpath("/html/body/div/div[3]/div/form/div[10]/div[5]/table[2]/tbody/tr[2]/td").join("\t\t")
        #results.last.insert(results.last.index("\t"), "\t")
        #results.last.insert(results.last.rindex(/\t\t\d\t\t\d/), "\t")
    ...
    def traceroute
        ...
        @ff.link(:id, "test_btn").click
        self.please_wait
        results << @ff.elements_by_xpath("/html/body/div/div[3]/div/form/div[8]/table[2]/tbody/tr/th").join("\t")
        while @ff.element_by_xpath("/html/body/div/div[3]/div/form/div[8]/table[2]/tbody/tr[#{count}]/td").exists?
            results << @ff.elements_by_xpath("/html/body/div/div[3]/div/form/div[8]/table[2]/tbody/tr[#{count}]/td").join("\t")
            count += 1
        end
        results.each { |x| @log.info "Traceroute::#{x}" }
    end
    ...
    def web_activity_log
        ...
        if @user_choices[:web_activity_log].nil?
            ...
            while @ff.element_by_xpath("/html/body/div/div[3]/div[2]/form/table/tbody/tr[#{count}]/td[2]").exists?
                @log.info "Web Activity Log::#{@ff.elements_by_xpath("/html/body/div/div[3]/div/form/div[2]/table/tbody/tr[2]/td").join("\t")}"
                count += 1
            end
