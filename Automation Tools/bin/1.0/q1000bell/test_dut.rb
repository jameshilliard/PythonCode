...
class Testing < UserChoices::Command
    ...
    def initialize(file="")
        ...
        @menu_links = {
            ...
            :advanced_setup => {
                ...
                :wan_ethernet_settings => "advancedsetup_waneth",
    ...
    def firewall
        ...
        ports = {
            ...
            :t120 => "1503",
            :telnet => "23",
            :vmc => "5500,5800-5801,5900-5901",
            :wii => "28910,29900-29901,29920",
            :wm => "1024-1030",
            :ws => "135-139,445,1434",
        ...
        if @ff.radio(:id, "firewall_security_level_off").checked?
        ...
        in_scan_rules.each_index do |i|
            ...
            scan_threads << Thread.new { @log.debug("Scanning: [#{i}] #{in_scan_rules[i]}"); scan_results << `ruby $SQAROOT/bin/1.0/q1000bell/tools/portscan.rb #{in_scan_rules[i]}`; @log.debug("Finished: [#{i}] #{in_scan_rules[i]}") }
        end
        ...
        out_scan_rules.each_index do |i|
            ...
            scan_threads << Thread.new { @log.debug("Scanning: [#{i}] #{out_scan_rules[i]}"); scan_results << `ruby $SQAROOT/bin/1.0/q1000bell/tools/portscan.rb #{out_scan_rules[i]}`; @log.debug("Finished: [#{i}] #{out_scan_rules[i]}") }
        end
    ...
    def port_forwarding
        ...
        while @ff.element_by_xpath("/html/body/div/div[4]/div[2]/form/table[3]/tbody/tr[#{table_count}]/td").exists?
            all_rules << @ff.elements_by_xpath("/html/body/div/div[4]/div[2]/form/table[3]/tbody/tr[#{table_count}]/td").join(";")
            table_count += 1
        end
    ...
    def port_forwarding_test(rules)
        ...
        scan_rules.each do |port_scan|
            scan_threads << Thread.new { scan_results << `ruby $SQAROOT/bin/1.0/q1000bell/tools/portscan.rb #{port_scan}` }
        end
        ...
        if @user_choices[:iperf]
            ...
            iperf_rules.each_index do |i|
                scan_threads << Thread.new { @log.debug("Testing: [#{i}] #{iperf_rules[i]}"); scan_results << `ruby $SQAROOT/bin/1.0/q1000bell/tools/iperf_test.rb #{iperf_rules[i]}`; @log.debug("Finished: [#{i}] #{iperf_rules[i]}") }
            end
    ...
    def nat
        ...
        if @ff.radio(:id, "nat_on").checked?
            ...
            scan_results << `ruby $SQAROOT/bin/1.0/q1000bell/tools/iperf_test.rb --protocol TCP --port 6000 --local_client --no-local_server --target 10.10.10.1`
            ...
            scan_results << `ruby $SQAROOT/bin/1.0/q1000bell/tools/iperf_test.rb --protocol UDP --port 6000 --local_client --no-local_server --target 10.10.10.1`
            ...
            scan_results << `ruby $SQAROOT/bin/1.0/q1000bell/tools/iperf_test.rb --protocol TCP --port 6000 --local_client --no-local_server --target 10.10.10.1 --no-data`
            ...
            scan_results << `ruby $SQAROOT/bin/1.0/q1000bell/tools/iperf_test.rb --protocol UDP --port 6000 --local_client --no-local_server --target 10.10.10.1 --no-data`
    ...
    def test_dut
        begin
            if @user_choices.key?(:after_test)
                ...
                self.menu(:status)
                @modem_ip_address = @ff.elements_by_xpath("/html/body/div/div[4]/div[2]/table[3]/tbody/tr[11]/td[2]/div/span/strong")[0].innerHTML rescue @ff.elements_by_xpath("/html/body/div/div[4]/div[2]/form/table[2]/tbody/tr[4]/td[2]/span")[0].innerHTML
