...
if [ -z $node ]; then
	echo "WARN: Please assign the type of change"
	echo $usage
	exit 1
fi

if [ -z $value ] && [ -z $startip ]; then
    echo "WARN: Please assign destination value of change."
    echo $usage
    exit 1
fi

if [ $value ] && [ $startip ]; then
    echo "WARN: Please assign one kind destination value of change."
    echo $usage
    exit 1
fi
...
if [ $startip ]; then
    if [ -z $endip ] || [ -z $mask ] || [ -z $gateway ]; then
        echo "WARN: Please assign whole value of change."
        echo $usage
        exit 1
    fi
fi
...
if [ -z "$file" ]; then
	echo "WARN: Please assign input file."
	echo $usage
	exit 1
fi
...
case $node in
    ...
    *)
	echo $comment
	echo "WARN: please input correct type of change."
	echo $usage
	;;
