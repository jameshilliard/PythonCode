...
def run(self):
    ...
    while not self.kill_received and not dhcpdos:
        ...
        dhcp_discover = Ether(src=m, dst="ff:ff:ff:ff:ff:ff") / IP(src="0.0.0.0", dst="255.255.255.255") / UDP(sport=68,
                                                                                                               dport=67) / BOOTP(
            chaddr=[mac2str(m)], xid=myxid) / DHCP(
    ...
    def detect_dhcp(self, pkt):
        ...
        if DHCP in pkt:
            if pkt[DHCP] and pkt[DHCP].options[0][1] == 2:
                ...
                if verbose: print(
                    "DHCPOFFER detected from " + pkt[Ether].src, sip + " on " + interface + ", handing out IP: " + myip)
